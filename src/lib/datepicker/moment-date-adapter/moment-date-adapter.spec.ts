...
describe('MomentDateAdapter', () => {
  ...
  it('should get year', () => {
    expect(adapter.getYear(moment([2017,  0,  1]))).toBe(2017);
  });

  it('should get month', () => {
    expect(adapter.getMonth(moment([2017,  0,  1]))).toBe(0);
  });

  it('should get date', () => {
    expect(adapter.getDate(moment([2017,  0,  1]))).toBe(1);
  });

  it('should get day of week', () => {
    expect(adapter.getDayOfWeek(moment([2017,  0,  1]))).toBe(0);
  });
  ...
  it('should get year name', () => {
    expect(adapter.getYearName(moment([2017,  0,  1]))).toBe('2017');
  });
  ...
  it('should create Moment date', () => {
    expect(adapter.createDate(2017, 0, 1).format())
      .toEqual(moment([2017,  0,  1]).format());
  });

  it('should not create Moment date with month over/under-flow', () => {
    expect(() => adapter.createDate(2017, 11 + 1, 1)).toThrow();
  ...
  it('should create Moment date with low year number', () => {
    expect(adapter.createDate(-1, 0, 1).year()).toBe(-1);
    expect(adapter.createDate(0, 0, 1).year()).toBe(0);
    expect(adapter.createDate(50, 0, 1).year()).toBe(50);
    expect(adapter.createDate(99, 0, 1).year()).toBe(99);
    expect(adapter.createDate(100, 0, 1).year()).toBe(100);
  });

  it('should not create Moment date in utc format', () => {
    expect(adapter.createDate(2017, 0, 5).isUTC()).toEqual(false);
  });
  ...
  it('should parse string according to given format', () => {
    expect(adapter.parse('1/2/2017', 'MM/DD/YYYY')!.format())
        .toEqual(moment([2017,  0,  2]).format());
    expect(adapter.parse('1/2/2017', 'DD/MM/YYYY')!.format())
        .toEqual(moment([2017,  1,  1]).format());
  });
  ...
  it('should format date according to given format', () => {
    expect(adapter.format(moment([2017,  0,  2]), 'MM/DD/YYYY')).toEqual('01/02/2017');
    expect(adapter.format(moment([2017,  0,  2]), 'DD/MM/YYYY')).toEqual('02/01/2017');
  });

  it('should format with a different locale', () => {
    expect(adapter.format(moment([2017,  0,  2]), 'll')).toEqual('Jan 2, 2017');
    adapter.setLocale('ja-JP');
    expect(adapter.format(moment([2017,  0,  2]), 'll')).toEqual('2017年1月2日');
  });
  ...
  it('should add years', () => {
    expect(adapter.addCalendarYears(moment([2017, 0, 1]), 1).format())
        .toEqual(moment([2018, 0, 1]).format());
    expect(adapter.addCalendarYears(moment([2017, 0, 1]), -1).format())
        .toEqual(moment([2016, 0, 1]).format());
  });

  it('should respect leap years when adding years', () => {
    expect(adapter.addCalendarYears(moment([2016, 1, 29]), 1).format())
        .toEqual(moment([2017, 1, 28]).format());
    expect(adapter.addCalendarYears(moment([2016, 1, 29]), -1).format())
        .toEqual(moment([2015, 1, 28]).format());
  });

  it('should add months', () => {
    expect(adapter.addCalendarMonths(moment([2017, 0, 1]), 1).format())
        .toEqual(moment([2017, 1, 1]).format());
    expect(adapter.addCalendarMonths(moment([2017, 0, 1]), -1).format())
        .toEqual(moment([2016, 11, 1]).format());
  });

  it('should respect month length differences when adding months', () => {
    expect(adapter.addCalendarMonths(moment([2017, 0, 31]), 1).format())
        .toEqual(moment([2017, 1, 28]).format());
    expect(adapter.addCalendarMonths(moment([2017, 2, 31]), -1).format())
        .toEqual(moment([2017, 1, 28]).format());
  });

  it('should add days', () => {
    expect(adapter.addCalendarDays(moment([2017, 0, 1]), 1).format())
        .toEqual(moment([2017, 0, 2]).format());
    expect(adapter.addCalendarDays(moment([2017, 0, 1]), -1).format())
        .toEqual(moment([2016, 11, 31]).format());
  });
  ...
  it('should compare dates', () => {
    expect(adapter.compareDate(moment([2017, 0, 1]), moment([2017, 0, 2]))).toBeLessThan(0);
    expect(adapter.compareDate(moment([2017, 0, 1]), moment([2017, 1, 1]))).toBeLessThan(0);
    expect(adapter.compareDate(moment([2017, 0, 1]), moment([2018, 0, 1]))).toBeLessThan(0);
    expect(adapter.compareDate(moment([2017, 0, 1]), moment([2017, 0, 1]))).toBe(0);
    expect(adapter.compareDate(moment([2018, 0, 1]), moment([2017, 0, 1]))).toBeGreaterThan(0);
    expect(adapter.compareDate(moment([2017, 1, 1]), moment([2017, 0, 1]))).toBeGreaterThan(0);
    expect(adapter.compareDate(moment([2017, 0, 2]), moment([2017, 0, 1]))).toBeGreaterThan(0);
  });

  it('should clamp date at lower bound', () => {
    expect(adapter.clampDate(
        moment([2017, 0, 1]), moment([2018, 0, 1]), moment([2019, 0, 1])))
        .toEqual(moment([2018, 0, 1]));
  });

  it('should clamp date at upper bound', () => {
    expect(adapter.clampDate(
        moment([2020, 0, 1]), moment([2018, 0, 1]), moment([2019, 0, 1])))
        .toEqual(moment([2019, 0, 1]));
  });

  it('should clamp date already within bounds', () => {
    expect(adapter.clampDate(
        moment([2018, 1, 1]), moment([2018, 0, 1]), moment([2019, 0, 1])))
        .toEqual(moment([2018, 1, 1]));
  });
  ...
  it('returned Moments should have correct locale', () => {
    adapter.setLocale('ja-JP');
    expect(adapter.createDate(2017, 0, 1).locale()).toBe('ja');
...
describe('MomentDateAdapter with LOCALE_ID override', () => {
  ...
  it('should take the default locale id from the LOCALE_ID injection token', () => {
    expect(adapter.format(moment([2017,  0,  2]), 'll')).toEqual('2 janv. 2017');
  });
});
...
describe('MomentDateAdapter with MAT_MOMENT_DATE_ADAPTER_OPTIONS override', () => {
  ...
  describe('use UTC', () => {
    it('should create Moment date in UTC', () => {
      expect(adapter.createDate(2017, 0, 5).isUtc()).toBe(true);
    });
