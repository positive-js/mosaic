...
task(
    'tslint:json',
    execNodeTask('tslint', [...tsLintBaseFlags, '--format', 'json', '--out', './dist/reports/tslint.json'])
);

task(
    'stylelint',
    execNodeTask('stylelint', [...styleGlobs])
);

task(
    'stylelint:json',
    execNodeTask('stylelint', [...styleGlobs, '-f', 'json', '--output-file', './dist/reports/stylelint.json'])
);
