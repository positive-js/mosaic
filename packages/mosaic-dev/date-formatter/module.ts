...
export class DemoComponent {
    ...
    private populateRelativeShort(locale: string) {
        ...
        relativeShort.tomorrow = this.dateFormatter.relativeLongDate(now.plus({ days: 1 }));
        relativeShort.fewDaysDiff = this.dateFormatter.relativeShortDate(now.minus({ days: 2 }));
        relativeShort.notCurrentYear = this.dateFormatter.relativeShortDate(
    ...
    private populateRelativeLong(locale: string) {
        ...
        relativeLong.fewDaysDiff = this.dateFormatter.relativeLongDate(now.minus({ days: 2 }));
        relativeLong.notCurrentYear = this.dateFormatter.relativeLongDate(
            now.minus({ years: 1, days: 2 })
        );
    }
