...
describe('AriaDescriber', () => {
  ...
  it('should not register non-string values', () => {
    expect(() => ariaDescriber.describe(component.element1, null!)).not.toThrow(); //tslint:disable-line
    expect(getMessageElements()).toBe(null);
  });

  it('should not throw when trying to remove non-string value', () => {
    expect(() => ariaDescriber.removeDescription(component.element1, null!)).not.toThrow(); //tslint:disable-line
  });
...
function getMessageElements(): Node[] | null {
  ...
  if (!messagesContainer) { return null; } //tslint:disable-line
...
function expectMessages(messages: string[]) {
  ...
  expect(messages.length).toBe(messageElements!.length); //tslint:disable-line
  messages.forEach((message, i) => {
    expect(messageElements![i].textContent).toBe(message); //tslint:disable-line
  });
}
...
function expectMessage(el: Element, message: string) {
  ...
  const messages = ariaDescribedBy!.split(' ').map((referenceId) => { //tslint:disable-line
